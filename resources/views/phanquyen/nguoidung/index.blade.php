  ...
  <table class="table table-bordered">
    ...
    <tbody>
     ...
     @foreach($nguoidungs as $nguoidung)
        <tr>
        	<td>{{$loop->index+1}}</td>
        	<td><img style="width: 100px;" src="public/anh/{{$nguoidung->anh}}"></td>
        	<td>{{$nguoidung->username}}</td>
        	<td>{{$nguoidung->namclass}}</td>
        	<td> {{date('d-m-Y', strtotime($nguoidung->ngaySinh))}}</td>
        	<td>{{$nguoidung->phongban->tenPhongBan}}</td>
       
        
        <td>


        	<a title="Sửa" href="{{route('suanguoidung',[$nguoidung->id])}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>  
        	<a title="Xóa" onclick="return xacnhanxoa('Bạn có chắc chắn muốn xóa không')"  href="{{route('xoanguoidung',[$nguoidung->id])}}"><i class="fa fa-trash" aria-hidden="true"></i></a>
